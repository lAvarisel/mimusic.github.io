...
let pugOption = {
  data: { repoUrl: 'https://github.com/lAvarisel/mimusic.github.io', },
...
function copyMedia(cb) {
  let copiedImages = [];
  nth.blocksFromHtml.forEach(function(block) {
    let src = `${dir.blocks}${block}/media`;
    if(fileExist(src)) copiedImages.push(src);
  });
  nth.config.alwaysAddBlocks.forEach(function(block) {
    let src = `${dir.blocks}${block}/media`;
  ...
  if(copiedImages.length) {
    (async () => {
      await cpy(copiedImages, `${dir.build}media`);
      cb();
    })();
  }
  else {
...
function serve() {
  ...
  watch([`${dir.blocks}**/media/*.{mp3}`], { events: ['all'], delay: 100 }, series(
    copyMedia,
    reload,
  ));
...
exports.build = series(
  ...
  parallel(copyMedia, copyImg, writeSassImportsFile, writeJsRequiresFile),
  parallel(compileSass, buildJs),
);
